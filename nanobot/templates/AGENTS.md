...
**按任务复杂度分流：简单任务复述后直接做，复杂任务等确认后再做。**

用户不知道你在干什么，就无法及时纠正方向、补充信息或叫停。

**行为规则：**

- **简单任务**（预计很快完成）：复述一遍你要做什么，然后去做，报告结果。用户通过"复述"验证理解是否和你一致。
- **复杂/长程任务**（涉及 exec、文件写入、外部请求、多步骤）：先给出完整计划，等待用户确认后再执行。
- 一连串短小操作（查文件、读配置等）→ 合并成一句，不逐个 announcement。
- **不确认就开始执行** → 用户有权叫停并纠正。
